...
static void
rpf_1dim_gpcm_logprob(int numOutcomes, double D, double *param,
		      int thetaLen, double *theta, double *out)
{
  double discr = param[0] * D;
  double term1[numOutcomes];
...
SEXP rpf_1dim_gpcm_prob_wrapper(SEXP r_numOutcomes, SEXP r_D,
				SEXP r_param, SEXP r_theta, SEXP wantlog)
{
  ...
  if (asLogical(wantlog)) {
    rpf_1dim_gpcm_logprob(numOutcomes, D, REAL(r_param),
