##' Mental silence dataset
##'
##' These data are from Pritikin & Schmidt (2013, March).
##' 
##' Items were fit to the Generalized Partial Credit Model (GPCM) with
##' standard Gauss-Hermite quadrature of 30 points and a lognormal
##' prior for the discrimination parameters using Parscale 4.1.2328.4,
##' -2 log likelihood = 20816.14.
##'
##' @docType data
##' @keywords datasets
##' @name mentalsilence
##' @aliases ms.items
##' @aliases ms.people
##' @source \url{http://openscienceframework.org/project/fpxmZ/}
##' @references Pritikin, J. N., & Schmidt, K. (2013, March). A
##' self-report measure for familiarity with mental silence.  Accepted
##' for Spirituality in the 21st Century: At the Interface of Theory,
##' Praxis and Pedagogy, Lisbon, Portugal.
##' @examples
##' data(ms.items)
##' data(ms.people)
NULL
