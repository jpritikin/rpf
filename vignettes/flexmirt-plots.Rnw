...
%\VignetteIndexEntry{Basic 1 dimensional plots}
...
This template is suitable for plotting a single group
of 1 dimensional items.
Refer to the Rnw source code to see how to adapt this
template to your project.

\section{Load data}

Normally you would obtain model parameters from OpenMx
(or flexMIRT with flexmirt.read), but here are some inline
...
\section{Item characteristic curve plots}
...
\section{Item information curve plots}

<<fig.height=5>>=
do.call(grid.arrange, c(small$iif, ncol=2))
@ 

\section{Whole test plots}
